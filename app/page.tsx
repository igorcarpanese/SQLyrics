...
export default function Home() {
  ...
  useEffect(() => {
    ...
    acDebounceRef.current = setTimeout(() => {
      fetch(
        `/api/artists?q=${encodeURIComponent(artistQuery)}&mode=${mode}`
      )
        .then((r) => r.json())
  ...
  // Highlight the matched portion in a suggestion
    ...
    <div className="min-h-screen bg-[#0a0a14] text-slate-200 font-sans">
      ...
      <header className="sticky top-0 z-50 border-b border-white/5 bg-[#0a0a14]/90 backdrop-blur-md">
        <div className="mx-auto max-w-5xl px-4 py-4">
          {/* Logo + mode toggle row */}
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <span className="text-2xl">🎤</span>
              <div>
                <h1 className="text-lg font-bold tracking-tight bg-gradient-to-r from-violet-400 to-pink-400 bg-clip-text text-transparent">
                  SQLyrics
                </h1>
                <p className="text-xs text-slate-500">Karaoke Song Browser</p>
              </div>
            </div>
            ...
            <div className="relative flex items-center bg-white/5 rounded-full border border-white/10 p-1 gap-0.5">
              ...
              <button
                ...
                className={`relative z-10 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-medium transition-colors duration-300 ${mode === "prefix" ? "text-white" : "text-slate-400 hover:text-slate-300"
                  }`}
              >
              ...
              <button
                ...
                className={`relative z-10 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-medium transition-colors duration-300 ${mode === "anywhere" ? "text-white" : "text-slate-400 hover:text-slate-300"
                  }`}
              >
                <span className="font-mono tracking-tight">·ab·</span>
                Anywhere
          ...
          <div className="flex gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                autoComplete="off"
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500/50 transition"
              />
                ...
                >
                  {suggestions.map((name, idx) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left transition-colors ${idx === activeSuggestion
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        {/* Table */}
        <div className="rounded-2xl border border-white/8 overflow-hidden bg-white/[0.02] shadow-2xl">
        ...
        {totalPages > 1 && (
          <div className="mt-6 flex items-center justify-center gap-1.5">
            <button
              onClick={() => goTo(page - 1)}
              disabled={page === 1}
              className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition text-sm"
            >
              ← Prev
            </button>
            ...
            {pageNumbers().map((p, idx) =>
              p === "…" ? (
                ...
                <button
                  ...
                  className={`min-w-[36px] h-9 rounded-lg text-sm font-medium transition border ${p === page
